...
// Ensure extra mentor assessment columns exist (fees/behavior/motivation)
async function ensureMentorAssessmentColumns() {
  try {
    const cols = await prisma.$queryRawUnsafe<any[]>("PRAGMA table_info('Student')");
    const have = new Set(cols.map(c=> c.name));
    const statements: string[] = [];
    if (!have.has('feesStatus')) statements.push("ALTER TABLE 'Student' ADD COLUMN 'feesStatus' TEXT");
    if (!have.has('behaviorScore')) statements.push("ALTER TABLE 'Student' ADD COLUMN 'behaviorScore' INTEGER");
    if (!have.has('motivationScore')) statements.push("ALTER TABLE 'Student' ADD COLUMN 'motivationScore' INTEGER");
    if (!have.has('lastMentorAssessment')) statements.push("ALTER TABLE 'Student' ADD COLUMN 'lastMentorAssessment' DATETIME");
    for (const sql of statements) {
      try { await prisma.$executeRawUnsafe(sql); } catch { /* ignore */ }
    }
  } catch { /* ignore */ }
...
// NOTE: Specific routes like '/risk-trend' must come BEFORE '/:id' to avoid being captured by the param route.
...
async function maybeAttachInferredRisk(rows: any[], tx?: any) {
  ...
  // Slightly rebalance default weights to emphasize attendance and assignments more
  let weights = { attendance: 0.4, gpa: 0.3, assignments: 0.25, notes: 0.05 };
  try {
  ...
  let notePenalty = 0;
  if (typeof r.mentorAcademicNote === 'string' && /fail|risk|struggl|drop|absent/i.test(r.mentorAcademicNote)) notePenalty = 0.2; // moderate boost
...
studentsRouter.get('/:id/360', authRequired, async (req: AuthedRequest, res) => {
  // Ensure dynamic academic columns exist (added at runtime outside Prisma schema)
  await ensureAcademicColumns();
  const { id } = req.params;
  // Allow mentors/counselors/admins, and allow student (viewer) to access ONLY their own record
  const isPrivileged = (isAdmin(req.user?.role) || isCounselor(req.user?.role) || isMentor(req.user?.role));
  const isSelfViewer = (req.user?.role === 'viewer' && req.user?.id === id);
  if (!(isPrivileged || isSelfViewer)) {
    return res.status(403).json({ ok: false, error: 'Forbidden', status: 403 });
  }
  try {
    const student = await prisma.student.findUnique({ where: { id } });
    if (!student) return res.status(404).json({ ok:false, error:'Not found' });
    // Fetch academic fields via raw SQL because Prisma schema may not include these runtime-added columns
    let extra: { attendancePercent?: number|null; cgpa?: number|null; assignmentsCompleted?: number|null; assignmentsTotal?: number|null; subjectsJson?: string|null; mentorAcademicNote?: string|null; lastAcademicUpdate?: string|null } = {} as any;
    try {
      const rows: any[] = await prisma.$queryRawUnsafe(
        `SELECT attendancePercent, cgpa, assignmentsCompleted, assignmentsTotal, subjectsJson, mentorAcademicNote, lastAcademicUpdate FROM "Student" WHERE id = ? LIMIT 1`,
        id
      );
      if (rows && rows.length) extra = rows[0] || {};
    } catch { /* ignore extras failure; fall back to nulls */ }
    ...
    // Fetch mentor name if assigned (best-effort)
    let mentorName: string | null = null;
    try {
      if (student.mentorId) {
        const mentor = await prisma.user.findUnique({ where: { id: student.mentorId }, select: { name: true } });
    ...
    return res.json({ ok:true, student: {
      ...
      guardianName: (student as any).guardianName ?? null,
      guardianEmail: (student as any).guardianEmail ?? null,
      guardianPhone: (student as any).guardianPhone ?? null,
      riskScore: student.riskScore,
      riskTier: deriveRiskTierFor(student.riskScore),
      lastRiskUpdated: student.lastRiskUpdated
    },
    academics: {
      attendancePercent: (typeof extra.attendancePercent === 'number') ? Number(extra.attendancePercent) : (extra.attendancePercent != null ? Number(extra.attendancePercent) : null),
      cgpa: (typeof extra.cgpa === 'number') ? Number(extra.cgpa) : (extra.cgpa != null ? Number(extra.cgpa) : null),
      assignmentsCompleted: (extra.assignmentsCompleted != null) ? Number(extra.assignmentsCompleted) : null,
      assignmentsTotal: (extra.assignmentsTotal != null) ? Number(extra.assignmentsTotal) : null,
      subjects: (()=>{ try { return extra.subjectsJson ? JSON.parse(extra.subjectsJson) : []; } catch { return []; } })(),
...
// POST /api/students/:id/dropout-assess
// Accepts mentor-provided factors and computes/updates risk score; returns suggested counseling messages
studentsRouter.post('/:id/dropout-assess', authRequired, async (req: AuthedRequest, res) => {
  await ensureAcademicColumns();
  await ensureMentorAssessmentColumns();
  await ensureRiskSnapshotTable();
  if (!(isAdmin(req.user?.role) || isMentor(req.user?.role))) {
    return res.status(403).json({ ok:false, error:'Forbidden' });
  }
  ...
  try {
    ...
    // Inputs: can be partial; fall back to existing
    const cgpaScale = Number(process.env.CGPA_SCALE) === 5 ? 5 : 10;
    const att = (typeof body.attendancePercent === 'number') ? Math.min(100, Math.max(0, body.attendancePercent)) : (student as any).attendancePercent ?? null;
    const gpa = (typeof body.cgpa === 'number') ? Math.min(cgpaScale, Math.max(0, body.cgpa)) : (student as any).cgpa ?? null;
    const fees = typeof body.fees === 'string' ? String(body.fees).toLowerCase() : ((student as any).feesStatus ?? null);
    const behavior = (typeof body.behavior === 'number') ? Math.min(10, Math.max(0, body.behavior)) : (student as any).behaviorScore ?? null;
    const motivation = (typeof body.motivation === 'number') ? Math.min(10, Math.max(0, body.motivation)) : (student as any).motivationScore ?? null;

    // Optional AI integration via external predictor
    let aiScore: number | null = null;
    let aiTier: string | null = null;
    const aiUrl = process.env.AI_PREDICT_URL || process.env.PK_AI_PREDICT_URL;
    if (aiUrl) {
      try {
        // Map available data to ML service features
        const avgTestScore = (typeof gpa === 'number') ? (gpa / cgpaScale) * 100 : ((student as any).avgTestScore ?? 60);
        const ac = (typeof (student as any).assignmentsCompleted === 'number') ? (student as any).assignmentsCompleted : 0;
        const at = (typeof (student as any).assignmentsTotal === 'number') ? (student as any).assignmentsTotal : 1;
        const controller = new AbortController();
        const t = setTimeout(() => controller.abort(), 1500);
        const resp = await fetch(String(aiUrl), {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            students: [{
              attendance_percentage: typeof att === 'number' ? att : ((student as any).attendancePercent ?? 70),
              avg_test_score: avgTestScore,
              assignments_submitted: ac,
              total_assignments: at || 1,
              fees_paid: fees === 'clear' ? 1 : 0
            }]
          }),
        ...
        if (resp.ok) {
          ...
          if (first) {
            const label = String(first.predicted_risk || '').toLowerCase();
            // Map labels to tier
            if (/high/.test(label)) aiTier = 'high';
            else if (/medium/.test(label)) aiTier = 'medium';
            else if (/low/.test(label)) aiTier = 'low';
            // Derive a score from probabilities if available, else tier-based default
            const probs = first.probabilities || {};
            const pHigh = Number(probs['High Risk'] ?? probs['high'] ?? 0);
            const pMed = Number(probs['Medium Risk'] ?? probs['medium'] ?? 0);
            const pLow = Number(probs['Low Risk'] ?? probs['low'] ?? 0);
            if (pHigh + pMed + pLow > 0) {
              aiScore = Math.max(0, Math.min(1, pHigh*0.85 + pMed*0.55 + pLow*0.2));
            } else if (aiTier) {
              aiScore = aiTier==='high'? 0.8 : aiTier==='medium'? 0.55 : 0.2;
            }
          }
        }
      } catch { /* ignore AI errors, fallback to heuristic */ }
    }
    ...
    if (typeof aiScore === 'number') {
      ...
      const attComp = att == null ? 0.5 : (1 - (att/100));
      const gpaComp = gpa == null ? 0.5 : (1 - (gpa/cgpaScale));
      const feesComp = fees == null ? 0.2 : (fees === 'clear' ? 0 : fees === 'due' ? 0.35 : 0.65);
      const behaviorComp = behavior == null ? 0.4 : ((10 - behavior) / 10); // lower behavior => higher risk
      const motivationComp = motivation == null ? 0.4 : ((10 - motivation) / 10); // lower motivation => higher risk
      const weights = { attendance: 0.25, gpa: 0.25, fees: 0.2, behavior: 0.15, motivation: 0.15 };
      score = attComp*weights.attendance + gpaComp*weights.gpa + feesComp*weights.fees + behaviorComp*weights.behavior + motivationComp*weights.motivation;
      score = Math.max(0, Math.min(1, score));
    }
    const tier = aiTier || deriveRiskTierFor(score);

    // Persist back to Student via raw SQL (columns may not exist in Prisma schema)
    const cols: string[] = []; const params: any[] = [];
    function push(col: string, val: any) { cols.push(`${col} = ?`); params.push(val); }
    if (att != null) push('attendancePercent', att);
    if (gpa != null) push('cgpa', gpa);
    if (fees != null) push('feesStatus', fees);
    if (behavior != null) push('behaviorScore', behavior);
    if (motivation != null) push('motivationScore', motivation);
    push('riskScore', score);
    push('lastRiskUpdated', new Date().toISOString());
    push('lastMentorAssessment', new Date().toISOString());
    params.push(id);
    if (cols.length) {
      try { await prisma.$executeRawUnsafe(`UPDATE "Student" SET ${cols.join(', ')} WHERE id = ?`, ...params); } catch {}
    }
    try {
      await prisma.$executeRawUnsafe(`INSERT INTO "RiskSnapshot" (id, studentId, riskScore, source) VALUES (?, ?, ?, ?)`, crypto.randomUUID(), id, score, 'mentor_assess');
    } catch {}

    // Counseling messages by tier (randomize a few)
    const pool: Record<string,string[]> = {
      high: [
        'Schedule an immediate one-on-one meeting to understand blockers and create a recovery plan.',
        'Engage parents/guardians and academic support services within this week.',
        'Prioritize attendance and essential coursework; set weekly checkpoints.',
        'Offer counseling and time-management workshop enrollment.'
      ],
      medium: [
        'Set short-term goals for attendance and assignments; review progress next week.',
        'Recommend study group or peer mentor pairing.',
        'Encourage regular check-ins and provide targeted resources for weak subjects.',
        'Share motivational tips and track consistency for two weeks.'
      ],
      low: [
        'Acknowledge good progress and suggest maintaining current routines.',
        'Encourage participation in extracurriculars to build confidence.',
        'Share strategies to sustain momentum before exam period.',
        'Invite to optional skill-building workshop.'
      ],
      unknown: ['Insufficient data; collect academic indicators and reassess.']
    };
    const msgs = (pool[tier] || pool.unknown);
    // pick up to 3 unique messages
    const shuffled = [...msgs].sort(()=> Math.random()-0.5);
    const counseling = shuffled.slice(0, Math.min(3, shuffled.length));

    return res.json({ ok:true, student:{ id, riskScore: score, riskTier: tier, lastRiskUpdated: new Date().toISOString() }, counseling });
  } catch (e:any) {
    return res.status(500).json({ ok:false, error:'Assessment failed', detail: e.message });
  }
});
