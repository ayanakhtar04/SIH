...
const App = React.forwardRef<AppRef, AppProps>(({ navOpen = true }, ref) => {
  ...
  // Refresh when students are claimed/assigned/edited elsewhere in the app
  useEffect(() => {
    const handler = () => fetchStudents();
    try { window.addEventListener('pk:students-updated', handler as any); } catch {}
    return () => { try { window.removeEventListener('pk:students-updated', handler as any); } catch {} };
  }, []);
